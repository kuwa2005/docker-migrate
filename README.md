...
| インポート | バンドル自動検出またはパス入力 → 内容プレビュー → オプション → 確認 → 実行 |
...
GUI では確認画面で「同名コンテナがあれば削除して上書き」を Y/n で選べます（既定: いいえ）。  
**いいえ** の場合、実行中に競合が検出されれば上記の **キーワード入力**（上書き / 複製）が表示されます。
